...
template <class T>
void lbag<T>::operator +=(const lbag & bag){
    lbag<T> nbag;
    list_copy(bag.head, bag.tail, nbag.head, nbag.tail);
    for(node<T> * p = nbag.head; p != nullptr; p = p -> link())
    {
        tail -> setlink_(p);
        tail = p;
    }
...
template <class T>
lbag<T> operator +(const lbag<T> & lhs, const lbag<T> & rhs){
    lbag<T> bag;
    bag = lhs;
    bag += rhs;
    return bag;
...
bool lbag<T>::erase_one(const T & value){
    ...
    {
        next = p -> link();
        if(p -> data() == value && prev == nullptr)
        {
            head = p -> link();
            delete p;
            return true;
        }
        else if(p -> data() == value && tail == p)
        {
            tail = prev;
            prev -> setlink_(next);
            delete p;
...
typename lbag<T>::size_type lbag<T>::erase(const T & value){
    if(erase_one(value) == false)
    {
        return 0;
    }
    else
    {
        return 1 + erase(value);
    }
}
