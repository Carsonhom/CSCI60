...
node::node(const value_type & initdata_, node * initlink)
{
...
void node::set_data(const value_type & newdata_)
{
    data_ = newdata_;
}
...
std::size_t list_size(const node * head){
    std::size_t count = 0;
     for(const node * p = head; p!=nullptr; p = p->link()){//p MUST be a const node to set it equal to a const node.
         count++;
     }
     return count;
}
